...
def dashboard():
    if 'username' in session:
        ...
        feedback = cursor.execute('SELECT * FROM feedback WHERE user_id = ?', (user_id,)).fetchall()

        # Fetch other relevant information based on user type

        conn.close()

        return render_template('dashboard.html', username=username, user_type=user_type, feedback=feedback) # other relevant data
