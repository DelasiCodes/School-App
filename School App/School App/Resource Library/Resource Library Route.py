...
def dashboard():
    if 'username' in session:
        ...
        resources = cursor.execute('SELECT * FROM resources WHERE user_id = ?', (user_id,)).fetchall()

        # Fetch other relevant information based on user type

        conn.close()

        return render_template('dashboard.html', username=username, user_type=user_type, resources=resources) #other relevant data
